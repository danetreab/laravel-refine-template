...
import {
  AuthenticatedUser,
  CheckRolePermissionResponseType,
  LoginResponseType,
  LoginType,
} from "@/types/auth-provider-type";
import { auth } from "@/api/auth";
import {
  LOCAL_USER_KEY,
  LOCAL_ACCESS_TOKEN,
  LOCAL_PERMISSIONS_KEY,
} from "@/config/local-storage-key";
...
export const authProvider: AuthProvider = {
  ...
  login: async ({
    phone,
    password,
  }: LoginType): Promise<AuthActionResponse> => {
  ...
  logout: async (params: any): Promise<AuthActionResponse> => {
    try {
      await axiosInstance.get<LoginResponseType>(API_URL + auth.logout);
    } catch (error) {
